				...
				<header id="cabecera" class="page-header animated fadeInLeft">
					...
					<div class="right-wrapper pull-right">
						<ol class="breadcrumbs">
							...
							<li><span>Mantenimiento de Unidades de Medida</span></li>
						</ol>
					...
					<div class="col-md-12">
						<section class="panel animated fadeInRight">
							<header class="panel-heading">
								<h2 class="panel-title">LISTA DE UNIDADES DE MEDIDA</h2>
							</header>

							<div class="panel-body">
								<div id="tabla">
									<table id="tblUnidadMed"
...
<!-- 										<a id="btnEliminar" class="btn btn-sm btn-primary width-default" onclick="preguntar();"><i class="fa fa-times"></i> ELIMINAR</a> -->
					...
					<div class="col-md-12" id="divUnidadMed" style="display: none">
						<form method="post" class="form-horizontal"  id="form" name="form">
							<input type="hidden" name="method" value="" /> 

							<section class="panel animated fadeInRight">
								<header class="panel-heading">
									<h2 class="panel-title">DATOS DE LA UNIDAD DE MEDIDA</h2>
								...
								<div class="panel-body">
									...
									<div class="form-group">
										<label class="col-sm-3 control-label"> UNIDAD DE MEDIDA <span class="required">*</span>
										</label>
										<div class="col-sm-9">
											<input type="text" id="txtNombre" name="txtNombre" class="form-control" maxlength="30" 
												onkeydown="upperCase(this)" title="Favor, Ingresar una Unidad de Medida" required />
										</div>
									</div>
								</div>
								<footer class="panel-footer">
...
<%-- 		<jsp:include page="plantillas/sidebar.jsp" /> --%>
	...
	function inicializarVariables(){
		var tableTest= $('#tblUnidadMed').DataTable();
		document.getElementById("divUnidadMed").style.display = "none";
	}
	
	function cargarComponentes(){
		$.ajax({
			url : "./cargarComponentesUnidadesMed",
			type : "POST",
			dataType : "json"
		})
		.done(
			function(paramJson) {
				...
				if (paramJson.estadoRespuesta == 'OK') {
					...
					loadTableUnidadMed(lstUnidadMed);
			...
			.done(
				function(paramJson) {
					...
					if (paramJson.estadoRespuesta == 'OK') {
						...
						limpiarText();
						callNotification('success','Se ha guardado satisfactoriamente los datos de la unidad de medida','Correcto');
						document.getElementById("divUnidadMed").style.display = "none";
						var lstUnidadMed = JSON.parse(paramJson.parametros.lstUnidadMed);
						loadTableUnidadMed(lstUnidadMed);
					} else {
	...
	function loadTableUnidadMed(lista){
		 
		 $('#tblUnidadMed').dataTable().fnClearTable();
		 var tblUnidadMed = $('#tblUnidadMed').DataTable(); 
		...
		if(lista.length > 0){
		 ...
		 $.each(lista,function(i,obj){
			 ...
			 var rowNode = tblUnidadMed.row.add( [
		...
		// Centrando columnas seleccion y codigo
		$('#tblUnidadMed').find('tr').each(function() {
	...
	function nuevo() {
		...
		.done(
			function(paramJson) {
				if (paramJson.estadoRespuesta == 'OK') {
					document.getElementById("divUnidadMed").style.display = "inline";
	...
	function eliminar(){
		...
		.done(
			function(paramJson) {
				if (paramJson.estadoRespuesta == 'OK') {
					callNotification('success','UnidadMed eliminado','Correcto');
					var lstUnidadMed = JSON.parse(paramJson.parametros.lstUnidadMed);
					loadTableUnidadMed(lstUnidadMed);
					document.getElementById("divUnidadMed").style.display = "none";
					cerrarModal();
